...
$danas->modify('+1 day');
           ...
           include("XMLSoccer.php");
            try{
                $soccer=new XMLSoccer("USZXLHGRDLBZSQXKJUZMVWTBZHUWHIJGEINSMYPRAPFQONVXII");
                ...
                $result=$soccer->GetFixturesByDateInterval(array("startDateString"=>$danas->format('Y-m-d')." 00:00","endDateString"=>$sutra->format('Y-m-d')." 00:00"));
                    foreach($result as $key=>$value){
                        ...
                        $tip1 = $odds->Odds->Bet_At_Home_Home;
                        $tip2 = $odds->Odds->Bet_At_Home_Away;
                        $tip_x = $odds->Odds->Bet_At_Home_Draw;
				if  (!empty($tip1)){
                        $game = $team1 ." - ". $team2;
							$insert_game = self::$db->query("INSERT INTO games (team1, team2, beginning, tip1, tip2, tip_x, utakmica) VALUES ('{$team1}', '{$team2}', '{$beginning}', '{$tip1}', '{$tip2}', '{$tip_x}', '{$game}')");
                    }
...
public static function EditGame(){
			...
			echo '<fieldset>';
				echo '<form action="" method="POST"/>';
					...
					echo '<input class="form-control" type="text" name="tip_win" value="'.$tip_win.'" placeholder="1 or 2 or X"/>';
			...
			echo '</fieldset>';
						if (isset($_POST['submit_edit'])) {
							if ($_POST['tip_win'] == 1 OR $_POST['tip_win'] == 2 OR $_POST['tip_win'] === 'X') {
							$team1_update = $_POST['team1'];
							$team2_update = $_POST['team2'];
							$tip_win_update = ucfirst($_POST['tip_win']);
							$utakmica="<br>".$tip_win_update. ": " .$team1_update. " - " .$team2_update.": ";
							...
							$update = self::$db->query("UPDATE winers SET g_match_w = REPLACE(g_match_w, '$utakmica1', '$utakmica')");
							$update2 = self::$db->query("UPDATE games SET tip_win = '{$tip_win_update}' WHERE id_game = '{$id}'");
								if (!$update2 == NULL) {
			...
			$status = 'Administrator';
				...
				?>
                                ...
                                <tr>
                                    ...
                                    <td style="color: #d9d9d9;"><?=$status?></td>
                                    <td><?=number_format($row['user_money'],2)?></td>
                                    <td style="color: #d9d9d9;"><?=String2Stars($row['email'],5,-5)?></td>
	...
	while ($row = $resultt->fetch(PDO::FETCH_ASSOC)){
		...
		if ($row['user_money'] > $virtual_money) {
			  ...
			  <?php
							...
							$sto= $translate['you_need']." ".number_format($money,2)." ". $translate['to_win']. " ".$prize."".$translate['and_high_rank'];
	...
	while ($row = $resultt->fetch(PDO::FETCH_ASSOC)){
		...
		if ($row['user_money'] > $virtual_money) {
			  ...
			  <?php
							...
							$sto= $translate['you_need']." ".number_format($money,2)." ". $translate['to_buy']. " ".$prize;
...
public static function AdminPrizeWiners(){
	...
	$prize_winers = self::$db->query("SELECT * FROM prize ORDER by prize_time ASC");
	$br = 0;
...
if ($row['prize_status'] == 0) {
	...
	<tbody>
		<tr>
	 <td><?=$br?></td>
	 <td><?=$row['prize_user_id']?></td>
	 <td><?=$row['prize_user']?></td>
	 <td><?=$row['prize_email']?></td>
	 <td><?=$row['prize_money']?></td>
	 <td>Payed</td>
	 <td><?=$time?></td>
	 <td><?php echo "<a style='{$hiden_style}' href='edit-prize-winer.php?id={$prize_id}'>Edit</a>";?></td>
...
public static function Options(){
   ...
   <?php
		if (isset($_POST['submit'])) {
						...
						$ad_update = $_POST['ad'];
						$analytics_update = $_POST['analytics'];
...
public static function ShowOrdersAdmin($limit){
	...
	$start_from = ($page-1) * $per_page;
		...
		if ($row['paid'] === '1') {
			...
			echo "<div id='pagination'><center><a href='users.php?page=1'>" . 'First page ' . "</a>";
			
			for ($i=1; $i<=$total_pages; $i++){
				echo "<a href='users.php?page=".$i."'>".$i."</a> ";
			};
			echo "<a href='users.php?page=".$total_pages."'>" . ' Last page' . "</a></div></center>";
}
...
public static function NotPaidWiners() {
	
		$result = self::$db->query("SELECT * FROM final_winers");
?>
		<table class="table table-bordered table-striped table-hover" id="table-bet">
...
<?php
	 ...
	 <td style="text-align: left;"><?=$row['win_id_user']?></td>
